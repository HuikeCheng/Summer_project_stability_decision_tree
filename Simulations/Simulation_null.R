...
#args <- c("100", "1000", "500", "5")
numrep <- as.numeric(args[1])
n <- as.numeric(args[2])
pk <- as.numeric(args[3])
nchunks <- as.numeric(args[4])

######## create folder
folder <- paste(paste0("Outputs/", "Null"), n, pk, sep = "_")
...
Simulation_study <- function(seed, n, pk, multivariate_normal = TRUE) {
  ##### set seed
  set.seed(seed)
  ##### simulate data
  # simulate xdata
  ...
  # simulate ydata
  ydata <- stats::rnorm(n = n, mean = 0, sd = 1)
...
#a <- Simulation_study(seed = 1, n = 1000, pk = 500)
...
clusterExport(cl, c("numrep", "n", "pk", "HugeAdjacency",
                    "CART1", "CART2", "getMetrics", "Simulation_study"))

out <- pblapply(1:numrep,
                function(i) {Simulation_study(seed = i, n = n, pk = pk)},
                cl = cl)
