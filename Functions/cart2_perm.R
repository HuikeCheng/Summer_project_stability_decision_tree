...
#'
#' @export
CART2 <- function(xdata, ydata, Lambda, family, maxsurrogate, ...) {
  ...
  # Change order of xdata columns
  rd <- sample(1:ncol(xdata), ncol(xdata))
  xdata_r <- xdata[,rd]
  
  # Writing the formula
  myformula <- stats::as.formula(paste0("ydata ~ ", paste(paste0("`", colnames(xdata_r), "`"), collapse = " + ")))
  
  # Creating the data frame
  mydata <- data.frame(ydata = ydata[, 1], xdata_r)
